...
export interface PlanDay {
  ...
  items: (string | null)[]
  note: string
  /** Markdown body of the matching "### <den> — ..." section, if any. */
  detail: string | null
}
...
export function parseWeekPlan(md: string, year: number): WeekPlan {
  ...
  // Day table: header row starts with "| Den |"; last column is a note.
  let columns: string[] = []
  const days: PlanDay[] = []
  let inTable = false
  ...
  // Daily detail sections: "### Po 6.7. — ..." — body until next heading.
  const detailRe = /^###\s+(.+?)\s*$/
...
export function parseLog(md: string, year: number): MonthLog {
  const lines = md.split('\n')
  const days: LogDay[] = []
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      current = { label, date: parseDayLabel(label, year), entries: [], sentence: '' }
      days.push(current)
      continue
    }
    if (!current) continue
